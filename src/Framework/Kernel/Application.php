...
use WebRover\Framework\Kernel\Container\ServiceProviderInterface;
...
abstract class Application extends Container implements KernelInterface, RebootableInterface, TerminableInterface
{
    ...
    public function registerModules()
    {
        ...
        if (!file_exists($configFile)) {
            throw new FileNotFoundException(sprintf('config file "%s" does not exist in path' . $this->configPath, 'app.php'));
        }

        $appConfig = include $configFile;

        if (!is_array($appConfig) || !isset($appConfig['module']) || !is_array($appConfig['module'])) {
            throw new \InvalidArgumentException('There is no module configuration available in the app file');
        }

        $registerModules = $appConfig['module'];

        foreach ($registerModules as $k => $v) {
            $registerModule = $v;

            if (class_exists($k)) {
                $registerModule = $k;
                if (!is_string($v) || !$v) {
                    throw new \InvalidArgumentException(sprintf('You must declare unique alias for "%s"', $k));
                }
            }
            ...
            if (!$module instanceof ModuleInterface) {
                $moduleReflection = new \ReflectionObject($module);
                throw new \LogicException('Module: ' . $moduleReflection->getName() . ' must is an instance of ModuleInterface');
            }

            $name = class_exists($k) ? $v : $module->getName();

            if (isset($this->modules[$name])) {
                if ($this->debug) {
                    throw new \LogicException(sprintf('A "%s" module had been declared', $name));
                } else {
                    continue;
                }
            }
    ...
    public function registerProviders()
    {
        ...
        if (isset($appConfig['provider']) && is_array($appConfig['provider'])) {
            $providers = array_merge($providers, $appConfig['provider']);
        }

        foreach ($this->modules as $module) {

            $registerProviders = $module->registerProviders();
