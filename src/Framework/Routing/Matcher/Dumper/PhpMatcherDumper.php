...
class PhpMatcherDumper extends MatcherDumper
{
    ...
    public function dump(array $options = [])
    {
        $options = array_replace([
            ...
            'base_class' => 'WebRover\\Framework\\Routing\\Matcher\\UrlMatcher',
        ], $options);

        // trailing slash support is only enabled if we know how to redirect the user
        $interfaces = class_implements($options['base_class']);
        $supportsRedirections = isset($interfaces['WebRover\\Framework\\Routing\\Matcher\\RedirectableUrlMatcherInterface']);
...
EOF;
    ...
    private function compileRoutes(RouteCollection $routes, $supportsRedirections)
    {
        ...
        $code .= "            throw new WebRover\Framework\Routing\Exception\NoConfigurationException();\n";
        $code .= "        }\n";
